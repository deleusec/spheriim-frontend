...
function CareerCard(props: CareerCardProps) {
  ...
  return (
    <div className="p-[16px] pb-[40px] h-full w-full rounded-xl shadow-cards flex flex-col gap-4 bg-white">
      <HeaderCards
        svg={<BriefcaseIcon className="w-[18px] h-[18px] text-white" />}
        title="Expérience"
      />
      <div className="flex flex-col justify-start gap-4">
        {experiences?.map((experience, index) => (
          <CareerContent
            key={index}
            logo={
              <BuildingOffice2Icon className="w-[18px] h-[18px] text-white" />
            }
            title={experience.company_name}
            jobPosition={experience.status}
            job={experience.job}
            dateStart={experience.start_date}
            dateEnd={experience.end_date}
